...
Oczywiście najfajnieszy, bo można go robić w na działającej bazie. 
Żeby móc backupwać bazę on-line, muszą być spełnione następujące warunki:

- [x] Włączona archiwizacja logów `ARCHMETH1` - nalepiej do Protecta
- [x] Cośtam jeszcze, jak sobie przypomnę to wpiszę.

Przykładowy skrypt backupowy:

```bash hl_lines="17" title="Skrypt backupu bazy"
--8<-- "scripts/db2-bkp.sh"
```